...
3. If AI vs AI is not running properly (eg. the field does not change when starting a play), `UFW` may be causing issues by blocking ports. `cd` into `Software/environment_setup` and run `./configure_ai_vs_ai.sh` to open the ports.
